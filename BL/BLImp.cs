...
namespace BL
{
    class BLImp : IBl
    {
        ...
        public int AddHostingUinit(HostingUnit toAdd)
        {
            ...
            try
            {
                key = dal.AddHostingUnit(Converters.Conv_BO_To_DO(toAdd));
                return key;
            }
            catch (Exception e)
            {
        ...
        public int AddOrder(Order newOrder)
        {
            ...
            try
            {
                ...
                key = dal.AddOrder(Converters.Conv_BO_To_DO(newOrder));
                return key;
            }
            catch (Exception e)
            {
        ...
        public void UpdateStatusOrder(BO.OrderStatus updateorder, int key)
        {
            try
            {
                BO.Order temp1 = Converters.Conv_DO_To_BO(dal.GetOrder(key));
                if (temp1.Status == OrderStatus.IGNORED_CLOSED || temp1.Status == OrderStatus.APPROVED)
                    throw new Exception("אין אפשרות לשנות את הסטטוס לאחר סגירת עסקה");
                BO.Host temp2 = Converters.Conv_DO_To_BO(dal.GetHost(temp1.HostId));
                ...
                if (updateorder == OrderStatus.MAIL_SENT && temp1.Status != OrderStatus.MAIL_SENT)
                {
                    try
                    {
                        SendEmail(temp1);
                        dal.UpdateStatusOrder(key, DO.OrderStatus.MAIL_SENT);
                    }
                    catch (Exception)
                    {
                ...
                if (updateorder == OrderStatus.MAIL_SENT && temp1.Status == OrderStatus.MAIL_SENT)
                {
                    throw new Exception("שלחת כבר מייל ללקוח ,אין אפשרות לשלוח שוב");
                }
                if (updateorder == OrderStatus.APPROVED)
                {
                    ...
                    update.TotalPrice = unit.PricePerNight * (request.LeaveDate - request.EntryDate).Days;
        ...
        public IEnumerable<HostingUnit> GetHostingUnitsByHostId(int id)
        {
            try
            {
                List<HostingUnit> units = dal.GetHostingUnits(x => x.HostId == id).Select(x => Converters.Conv_DO_To_BO(x)).ToList();
                return units;
            }
            catch (Exception e)
            {
        ...
        public void SendEmail(Order order)
        {
            try
            {
                ...
                mail.To.Add("yedidi08@gmail.com");
